...
const AddPage = ({ handlePageChange, handleBackClick }) => {
    ...
    const handleAlokasiChange = async (selectedOption) => {
        ...
        let nomorpo = 0;
        try {
            const response = await axios.get(`https://apipm.delapandelapanlogistics.com/api/v1/po/jumlah/kantor/${id_kantor}`, {
                headers: {
                    Authorization: token
                }
            });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`https://apipm.delapandelapanlogistics.com/api/v1/po`, dataToSubmit, {
                headers: {
                    Authorization: token
                }
            });
